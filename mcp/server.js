...
function buildCssPalette(palette) {
    const cssVars = palette.map(c => `--${c.name}: ${c.hex};`).join('\n  ');
    return `:root {\n  ${cssVars}\n}`;
}
function paletteForParams(params) {
...
function handleRequest(req) {
    ...
    if (req.method === 'generatePalette') {
        try {
            ...
            const palette = paletteForParams(params);
            const css = buildCssPalette(palette);
            const result = { content: [{ type: 'text', text: css }], meta: { theme: params?.theme ?? 'default', dark: params?.dark ?? false } };
